...
if(is_object($internaute) && isset($internaute->authlevel) && $internaute->authlevel >= 1 && $do == "deconnect")
{
    $message = $message . deconnection();
}

if(!is_object($internaute) || empty($internaute->repskin))
{
...
$prive = 0;
if(is_object($internaute) && isset($internaute->authlevel) && $internaute->authlevel >= 1) //accès restreint aux enregistrés
{
    $prive = 1;
    switch ($do)
    {
        ...
        case "delmessage":
            ...
            break;
        case "ajgroupe":
            if(defined('ACTIVATION_GROUPE') && ACTIVATION_GROUPE)
...
$admin = 0;
if(is_object($internaute) && isset($internaute->authlevel) && $internaute->authlevel > 1) //ADMIN OU MODOS
{
    $admin = 1;
...
if($public == 0 && $prive == 0 && $admin == 0 && $do <> "deconnect")
{
    echo pgaccueil(sec(intval($numligne)));
}
